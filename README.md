...
[![Version](https://img.shields.io/badge/version-v0.6.40-blue.svg)](portable-spec-kit.md)
...
**v0.6.40 (Playwright server-lifecycle, Next.js colocated template, template-strict scaffold, cycle persistence, cascade Kit field):** Five structural improvements. **server-lifecycle.sh (NEW):** starts dev server before QA-Agent; port poll via `nc -z`; PID to `.server.pid`; process group kill for webpack/esbuild children. `spawn-qa.sh` + `run.sh` lifecycle hooks + `reflex/config.yml server_lifecycle:` block (opt-in, `enabled: false` default) + `qa-agent.md REFLEX_SERVER_STATUS` awareness — QA never runs Playwright without a live server. **source-structures.md:** Next.js Full-Stack (colocated) DEFAULT template — ALL app code under `src/`, `public/` at root (framework constraint), `drizzle.config.ts out: ./src/db/migrations`; Stack matching rule + Monorepo template added. **psk-orchestrate.sh Phase 6:** template-strict scaffold reads PLANS.md Stack → matches exact template → creates dirs exactly per template. **reflex/run.sh:** `.active-cycle` state file persists cycle-id across bash invocations; cleared on GRANTED. **psk-version-cascade.sh Phase Q:** updates `**Kit:**` field in sync-target AGENT_CONTEXT.md. 1982 tests (1837+145). See [CHANGELOG.md](CHANGELOG.md) for full history.
