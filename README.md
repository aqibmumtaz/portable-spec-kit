...
[![Version](https://img.shields.io/badge/version-v0.6.99-blue.svg)](portable-spec-kit.md)
[![License: MIT](https://img.shields.io/badge/License-MIT-blue.svg)](LICENSE)
[![Tests](https://img.shields.io/badge/tests-3165%20passing-brightgreen.svg)](tests/)
...
Installs: `portable-spec-kit.md` (~1100 lines) · 64 `psk-*` reliability scripts + 3 helper scripts (install-tracker, sync, uninstall-tracker) — 67 total `.sh` files in `agent/scripts/` · 29 skill files (JIT-loaded) · 25 file-class templates (`.portable-spec-kit/templates/`) · 34 flow documents + 1 template (`docs/work-flows/`, 35 `.md` files) · 4 stack-aware CI templates · PreCommit + PostToolUse hooks · symlinks for all supported agents.
...
**v0.6.99 (Structural chat-progress guarantee + maintainer-backlog drain):** The chunked-drive box table is the only progress surface every client renders, but driving a long op through it had been agent-driven — an agent could run a long op as a plain Bash call and the chat would show nothing. **KIT-GAP-0147** closes that structurally: a new `agent/scripts/psk-chunked-drive-guard.sh` PostToolUse(Bash) hook detects ANY long op invoked directly (keys off the canonical `# long-op:` marker plus `reflex/run.sh` / `psk-release.sh prepare|refresh` / `sync-check --full`) and injects a chat-surfaced nudge to re-drive it chunked. It is generic — a new long op is covered the moment it gets its `# long-op:` header — and fires outside agent control (Layer-3 hook). New sync-check **PSK051** verifies the guard is present + wired as a PostToolUse hook and that every `# long-op:` script is chunk-reachable; `psk-install-hooks.sh` wires the guard into the project + nested-workspace settings. The 43-entry maintainer backlog was triaged to closure: 6 fix-worthy tasks landed — **G43** (§Reliability "11 structural checks" prose corrected to 56, guarded by a new **PSK050** prose-claim drift gate), **G25** (`psk-workflow-watchdog.sh` surfaces `in_progress`/`running` phases), **G27** (`psk-kit-cmd.sh` log sanitization + `.kit-gap-log` repaired to canonical 5-field TSV), **G28** (`psk-dispatch.sh init` archives legacy orphan-state files), **G31** (`tests/features/f86-session-health.sh` gives F86 its R→F→T coverage), **G35** (`summary.csv` documents blank cost columns as uninstrumented). 13 tasks closed-resolved, 5 rejected, 4 kept as proposals; ADR-095 + ADR-096 recorded. **3165 tests passing (3020 framework + 145 benchmarking).**

**v0.6.98 (Reflex cycle-03 convergence — Dev-Agent fixes 10 scope:kit findings + monitor auto-render + redispatch phantom-spawn fix):** The reflex cycle-03 kit-self-test convergence pass. The QA wave surfaced 14 actionable findings (5 MAJOR); the Dev-Agent fixed 10 `scope:kit` findings end-to-end under kit-evolution mode (1 `scope:meta` routed to G43, 3 advisory deferred), all 15 mechanical gates passed green. Two convergence-blocking machinery gaps were closed inline. **KIT-GAP-0142** — `reflex/run.sh` `--resume-dims` detect-and-redispatch no longer stalls at AWAITING_DIM_DISPATCH on phantom spawn records: the spawn-parsing awk mis-read nested `prior_findings` ids (empty artifact + assigned_dims) as missing spawns, so the loop now skips any record lacking both fields. **KIT-GAP-0143** — `reflex/lib/progress-surface.sh` (the ONE shared emitter run.sh calls at both QA/Dev resume points) now AUTO-RENDERS the accurate `status --table` from the kit: it plans rows from the pass artifacts, drives the tracker deterministically (no off-by-one possible), fills each RESULT from artifacts (QA: per-dim finding count; Dev: per-finding disposition from `dev-result.md`), and emits the canonical table via the central renderer (PSK049-compliant). The cycle-03 Dev fixes added the new **PSK050** prose-claim drift gate (QA-D37) plus 9 more `scope:kit` fixes (EVAL_TRACE verdict read, Phase-0 warm caches, philosophy-gaps artifact, abort-recovery annotation, SPECS count, psk-spawn unbound-var guard, orchestration phase count, wave-plan naming). cycle-03/pass-001 verdict NEEDS_QA_VERIFY with all 15 gates green. **3165 tests passing (3020 framework + 145 benchmarking).**

**v0.6.97 (Reflex cycle-03 kit-self-test — generic single-sourced progress-table + structural chunked-drive guard + cycle-summary empty-cycle fix):** A reflex kit-self-test convergence cycle (cycle-03) surfaced three machinery gaps in the no-silent-wait progress system and the per-cycle aggregator — all fixed end-to-end with structural guards and regression tests. **KIT-GAP-0139** — `reflex/lib/cycle-summary.sh` `discover_cycles` now yields only cycles with ≥1 pass dir, so a `--next-cycle` advance that pins `.active-cycle=N` before cycle-N's first pass no longer fails the aggregate run with exit 1 (the explicit-`$1` path still returns non-zero for a named empty cycle). **KIT-GAP-0140** — `agent/scripts/psk-tests-gate.sh` gains a structural no-inline-fallback guard: `run` REFUSES the opaque inline suite (exit 7 + chunk-drive recipe) in an agent-driven context (no TTY, not CI, no `PSK_TESTS_GATE_ALLOW_INLINE=1`), forcing the chunked path so reflex/prepare surfaces per-section chat progress; CI/TTY/opt-in still run inline. **KIT-GAP-0141** — the progress-table template is now generic and single-sourced: each chunk carries a static `# stage:<name>` label and the ONE central renderer shows the `Stage` column only when the plan has ≥2 distinct stages (suppressed for a single-suite drive); `psk-spawn.sh` references the canonical `status --table` instead of hand-listing columns; new sync-check **PSK049** fails if any script but `psk-chunked-run.sh` hand-draws the box table; the No-Silent-Wait recipe now mandates pasting the table into the reply message body. **3165 tests passing (3020 framework + 145 benchmarking).**

**v0.6.96 (Kit-evolution Dev-fix path — the reflex Dev-Agent fixes scope:kit findings end-to-end):** The automated kit-evolution loop now actually works: the reflex Dev-Agent can fix `scope:kit` findings, gated by genericity guards, so a kit self-test converges via the Dev-Agent instead of routing every finding to a human. Driving the loop empirically surfaced three machinery gaps never reached before. **KIT-GAP-0129** — `reflex/lib/kit-evolution.sh` Phase 3 is now PAUSE-AWARE: `run.sh` exit 2/3 (AWAITING the multi-author QA spawn) is a pause the main agent drives, not a failure; it records pause state, propagates exit 2, and re-entry is idempotent. **KIT-GAP-0130** — `REFLEX_KIT_EVOLUTION=1` now AUTHORIZES the Dev-Agent to fix `scope:kit` findings (per-pass prompt injection in `spawn-dev.sh`), gated strictly on the env var so normal user-project passes still route upstream; §Reflex-Finding-Classification carries the matching exception. **KIT-GAP-0131** — the G1 file-scope gate was redesigned from a fragile enumerated-subdir allowlist (it false-flagged `ard/`, `agent/tasks/`, `reflex/history/` when the kit is nested in a parent repo, and `--relative` hid sibling bleed) to a kit-prefix check via `git rev-parse --show-prefix` — only paths OUTSIDE the kit root are project-bleed. With all three closed, the Dev-Agent fixed 12 `scope:kit` findings (jq dependency removed, audit-probe precision, pipefail hardening, +7 scripts test-covered via new `tests/sections/98-script-coverage.sh`, supply-chain A08 doc) and all 14 mechanical gates passed; 4 net-new-feature findings routed to non-blocking backlog. **3165 tests passing (3020 framework + 145 benchmarking).**

**v0.6.95 (Fail-closed tests pre-verify gate — in-chat progress for reflex/prepare):** Running reflex/prepare now surfaces the test suite per-section in the chat, with a fail-closed safety gate so nothing untested can ship. The release ceremony used to run the full Test Execution Flow as one opaque ~25-minute background op, and the chat is turn-based so a single background op cannot stream progress into the chat (KIT-GAP-0112). **KIT-GAP-0123** — new `agent/scripts/psk-tests-gate.sh` decouples the suite from prepare's `step-1-tests`: the agent drives the suite CHUNKED first (per-section chat progress via `psk-chunked-run.sh`), each test process writes its own real exit-0 STAMP, and on all-green the gate SEALS a fail-closed pre-verify marker keyed to a full tree fingerprint (`HEAD` + working tree); `step-1-tests` then routes through `psk-tests-gate.sh run`, which SKIPS the inline re-run only when the marker proves the current tree (no double-run, cf. KIT-GAP-0121). Any code/test change, dirty tree, missing/failed unit, or non-git → tests run. **KIT-GAP-0123-HARDENED** — an adversarial review (4 lenses) of the first HEAD-SHA design caught 3 CRITICALs (dirty-tree, narrative-seal, zero-results); redesigned fail-closed and proof-based. **KIT-GAP-0124** — `PSK_TESTS_GATE_STATE_DIR` isolates section-109's gate exercise from production stamps during a live drive. **PSK048** enforces the wiring (gate + step-1 routing + reflex Phase-1 chunk-drive). **KIT-GAP-0116** wires reflex QA/Dev progress through the centralized `psk-chunked-run.sh`. **3165 tests passing (3020 framework + 145 benchmarking).**

**v0.6.94 (No-silent-wait centralized progress — chunked-drive + status --table + whole-suite coverage):** Every long blocking op now surfaces progress as chat messages through one centralized driver. Chat messages are the only progress surface every client renders — the Claude Code `statusLine` bar is CLI-terminal-only (the VS Code extension / app UI does not render it) — so chunked-drive is now the DEFAULT for every long op. **KIT-GAP-0113** — `agent/scripts/psk-chunked-run.sh` (new, the ONE centralized driver) runs a long op as a series of short background tasks, and each chunk-completion becomes a chat turn that prints the result + launches the next chunk (`--suite test-spec-kit`/`all-tests`/`prepare-release`; `status --table` renders the canonical `| Chunk | Unit | Result |` table). **KIT-GAP-0114** — the `--suite test-spec-kit` derivation chunked only sections, silently omitting the 85 feature files (~430 tests); now covers the whole suite. **KIT-GAP-0109/0110/0111** hardened `psk-progress.sh` (statusLine surface, TMPDIR-independent live dir, instant write). **PSK047** now also requires the chunk driver present; new flow doc `34-no-silent-wait.md`; §No-Silent-Wait + §Session Health framing flipped so the breadcrumb (chat) is the guarantee and the statusLine is a CLI-only complement. **3165 tests passing (3020 framework + 145 benchmarking).**

**v0.6.93 (Session-health indicator — structural statusLine + UserPromptSubmit loop fix + authoritative ctx limit):** The `ctx:` context-health badge now appears structurally and reads the true context window. **KIT-GAP-0097** — the monitor shipped (v0.6.90) as a `Stop` hook, but a Stop hook that returns `additionalContext` re-invokes the agent every turn (infinite loop), so the badge looped instead of rendering. Migrated to a `UserPromptSubmit` hook (fires once per real user turn) plus a Claude Code `statusLine` — the always-on structural surface that renders `ctx:` (+`opt:`) every turn independent of the agent. `resolve_limit` now derives the context window authoritatively from a `compact_boundary`'s `compactMetadata.preTokens` (a 200k window cannot hold a larger pre-compact size → proves a ≥1M window) instead of a `>220k` guess, removing the tier inversion and post-compact mis-tiering. The installer wires both surfaces in the project + nested-workspace settings (migrate-and-add strips any legacy `.hooks.Stop` wiring, never clobbers an operator's custom `statusLine`). **3165 tests passing (3020 framework + 145 benchmarking).**

**v0.6.92 (Multi-author reflex enforcement — structural inline-prevention + detect-and-redispatch):** Reflex multi-author QA can no longer silently collapse to inline synthesis. **KIT-GAP-0098** — `reflex/run.sh`'s orchestrator may write only planning artifacts (`wave-manifest.yaml`, dim-prompts); `findings.yaml` is synthesized solely from `partial-findings-dims-*.yaml` emitted by real dim-agents, and the dim-agent count is now dynamic (`ceil(dims / max_dims_per_spawn)`) instead of a hardcoded loop. **Follow-up** — detect-and-redispatch reads the wave manifest's `artifact:` path to resolve qa-prefixed spawn ids to their un-prefixed partial filenames, fixing a false-missing bug that re-dispatched every dim-set (regression test added in `tests/sections/04-reflex.sh`). **ADR-091** records the decision. **Flow-doc currency** — `docs/work-flows/{06,11,13}` PSK rule range synced PSK001-PSK045 → PSK001-PSK046 (50 checks). First end-to-end multi-author reflex self-test (cycle-01/pass-001) drove orchestrator → 4 parallel dim-agents → synthesis → Dev → 14 gates; gate-13 confirmed a genuine adversarial audit. **3070 tests passing (2925 framework + 145 benchmarking).**
...
**v0.6.72 (QA-Orchestrator Multi-Author Dispatch):** Closes 6-pass MAJOR QA-ORCH-COVERAGE finding via §Spawn Fidelity. **KIT-GAP-0052** — `psk-spawn.sh request-multi` + `complete-multi` primitives for one-phase-N-parallel-spawns via YAML manifest. `qa-agent-orchestrator.md` three-path dispatch (multi-author > Task-inline > single-author). `qa_agent.spawn_mode` config flag. Section 94 20-test regression. Verified end-to-end on cycle-26-pass-002 (3 parallel dim-agents → GRANTED via dispatch).

**v0.6.71 (PSK005 Recursion Guard):** Closes 3-way infinite recursion (2.6K+ procs, 8h gates.sh hang). **KIT-GAP-0051** — `test-release-check.sh` exports `PSK_IN_TEST_RELEASE_CHECK=1` sentinel; `check_rft_gate()` short-circuits when set. test-release-check.sh 8h hung → 24s clean.

**v0.6.70 (Sync-Check Perf Sweep + Gates Trap):** Sync-check wall-clock 60-90s → 23s clean. **KIT-GAP-0050** — provisional `verdict.md` pre-write before gates.sh runs (L2 EXIT trap sees verdict + skips INTERRUPTED stamp).

**v0.6.67 (Recursion Fixes — Legitimate-Exception Taxonomy):** Second production validation of the §Kit Fidelity friction-as-feedback loop. Cycle-22 reflex autoloop surfaced 4 recursive false-positives (PSK040, PSK041, Dim 27) + the KIT-GAP-0013 hook-installer mangling bug. v0.6.67 closes them via a shared **legitimate-exception taxonomy** so kit rules can distinguish operator-controlled non-pending states from real workaround anti-patterns. **PSK041 disposition enum** extended from `{pending, kit-fixed, escalated}` to `{pending, deferred, bypassed, outside-repo, escalated, kit-fixed}`. **psk-kit-cmd.sh --log-gap** — new `--defer <target>` / `--bypassed <flag>` / `--outside-repo <path>` flags. **PSK040 commit-shape whitelist** — legitimate release-ceremony + KIT-GAP-closer subjects (`release: vX.Y.Z`, `fix(KIT-GAP-NNNN):`, `vN.M.P:`, `chore(release):`, `docs(release):`) skip marker-pattern detection. **Dim 27 documented-fallback bypass** — `single_author_write_timestamp` skips when `qa-usage.yaml` declares `mode: single-author-fallback-documented`. **psk-install-hooks.sh hardening (closes KIT-GAP-0011 + KIT-GAP-0013)** — new `safe_install_hook()` with `bash -n` syntax check before atomic tmp→mv, restore from `.psk-backup` on failure, body validation before splicing. **6 flow docs synced to v0.6.66 counts** — 38 PSK rules (PSK001-PSK041), 13 mechanical gates, 28 reflex dimensions, 2720 framework tests, 85 features. **Defers** KIT-GAP-0012 → v0.6.68 (PSK031 dedup-vs-alias gap). **2865 tests** (2720 framework + 145 benchmarking), 0 failures.

**v0.6.66 (Kit-Machinery Hardening — 5 KIT-GAPs closed):** First production validation of the §Kit Fidelity friction-as-feedback loop. v0.6.64 shipped the layer; v0.6.65 exercised it across two release ceremonies + an autoloop debug session, surfacing 7 distinct kit-machinery bugs (filed as `KIT-GAP-NNNN` in `agent/.kit-gap-log`). v0.6.66 closes 5 at the kit-machinery level — no workarounds. **Added** — `psk-version-bump.sh` (patch-bump helper with per-run idempotency marker, closes KIT-GAP-0002/0007); `--next-cycle` escape hatch in `reflex/run.sh` (closes KIT-GAP-0006); PSK041 sync-check rule auditing pending-disposition markers (closes KIT-GAP-0008). **Changed** — `phases.yml step-6-version` now chains `psk-version-bump.sh && psk-version-cascade.sh`; `psk-workflow-state.sh verify-gate` uses two-stage quoting (closes KIT-GAP-0005); `--log-gap` writes pending markers + uses base-10 forced auto-increment. **2865 tests**, 0 failures.
...
**v0.6.57 (Plan Execution Protocol + UI Completeness Gate):** New MANDATORY §Plan Execution Protocol in `portable-spec-kit.md` — 5th reliability layer (plan-shape gate). Every plan executed via sub-agent spawning conforms to `schema_version: 1` + `phases:` frontmatter. New `agent/scripts/psk-run-plan.sh` (35th script) — schema-aware driver with commands start/next/status/resume/retry/--convert/--validate/--health/abort; no inline-fallback branch (structurally only spawn/retry/abort paths forward). Three new templates: `plan-executable.md` + `plan-prompt.md` + `plan-artifact.md`. `psk-plan-save.sh` body-preservation fix + schema validation on `approve` (PSK024 codes N/P/F/D/L). PSK024 sync-check rule (27th rule, 11 sub-codes V/P/I/N/R/A/G/C/D/M/X) lints every `agent/plans/*.md`. §Workflow Fidelity B2 retrofit: `register-gate`/`verify-gate` API on `psk-workflow-state.sh` + 9 workflows wired (`mark-done` refuses unverified phases). New `agent/scripts/psk-ui-completeness.sh` (36th script) — 10-category stack-aware UI audit (P primitives, L layout, D dark-mode, S states, A a11y, T tokens, F per-feature pages, I input-feedback, R responsive, E empty-shell), bash 3.2 compatible. PSK025 sync-check rule (28th rule). Orchestrator `--update` U6.5 — UI-completeness backfill phase that scaffolds missing primitives, layout, dark-mode, states, a11y, tokens, pages until `--strict` exits 0; no skip path under §Workflow Fidelity. Reflex Dim 26 + 12th mechanical gate `workflow-fidelity-completeness`. New flow docs 26 (Plan Execution Protocol) + 27 (UI Completeness Gate). New `.portable-spec-kit/skills/plan-execution.md`. Closes the v5-skeletal-UI failure structurally — empty-shell pages now auto-flagged as PSK025-E. **2325 tests** (2180 framework + 145 benchmarking).

**v0.6.56 (Workflow Fidelity — the agent executes the kit, never overrides it):** New MANDATORY §Workflow Fidelity section in `portable-spec-kit.md` — the 4th reliability layer, behavioral and structurally enforced. When the kit defines a process (workflow, phase, gate, rule, principle, test) the agent executes it faithfully and completely: no phase compression/skip/reorder, no inline substitution where a sub-agent is specified, no scope reduction under rate/context pressure, no replacing the kit's "done" with its own judgment. Rate/context limits → pause-and-resume, never reduce-scope. New principle `P10 — Workflow Fidelity` in `agent/PHILOSOPHY.md`. New `agent/scripts/psk-workflow-state.sh` (33rd script) — resumable phase-state machine; a phase cannot advance until its registered completion gate passes; `resume` re-enters at the exact in-progress phase. New `agent/scripts/psk-spawn.sh` (34th script) — sub-agent spawn protocol with NO inline-fallback branch; on rate-limit the only forward command is retry-spawn. 9 executable workflows retrofitted. New flow doc 25. 18 regression tests. Motivating incident: searchsocialtruth-v5 shipped a skeletal UI behind a "20/20 features, 110 tests" report — the agent overrode the kit's defined process under rate/context pressure. **2325 tests** (2180 framework + 145 benchmarking).

**v0.6.55 (Source Layout Policy + 10 new templates + flow doc 24):** Stack-driven source-layout policy — the orchestrator reads `agent/PLANS.md` Stack table → derives Template choice (1 colocated / 3 separate-services / 4 monorepo / 5 library / 6 CLI) + opt-in `src/` subdirs (`core` + `shared` always; `ui` / `api` / `integrations` / `platform` per Stack). New `psk-scaffold-src.sh` (32nd script). PSK022a (template choice) + PSK022b (src subdir layout) sync-check rules. 10 new file-class templates (23 total, all pass the 7-criterion Quality Bar). Flow doc 24 (template-driven creation). **2352 tests** (2207 framework + 145 benchmarking).

**v0.6.54 (Template Quality Bar):** New §Template Quality Bar rule — 7 mandatory criteria every kit-shipped template must pass (stack-agnostic, domain-agnostic, scale-agnostic, useful-and-complete, lifecycle-aware, self-documenting, round-trippable). `.portable-spec-kit/templates/` externalized templates dir (13 templates extracted from inline). New `agent/scripts/psk-template-quality.sh` (31st script) — 7-criterion lint with JSON output + `--strict` gate mode. PSK023 sync-check + `.audit-log.yaml`. **2337 tests** (2192 framework + 145 benchmarking).

**v0.6.53 (Plan-Save Protocol — interruption-resilient plan persistence):** New mandatory rule in `portable-spec-kit.md` §Task Tracking: any conversation-drafted plan (≥3 markdown headings OR ≥30 lines AND a plan-shaped prompt) MUST persist to `agent/plans/YYYY-MM-DD-<slug>.md` as the agent's first tool call of the same turn, before any implementation. Lifecycle frontmatter drives `draft → approved → executing → done | abandoned` so any interruption point leaves the latest plan version in git history. New script `agent/scripts/psk-plan-save.sh` (30th kit script): subcommands `save` / `approve` / `start` / `done` / `abandon` / `list` / `show`. Idempotent — re-saving rewrites body and refreshes `updated:` field while preserving status. Demonstrated by self-application: the v0.6.53 plan itself was saved as `agent/plans/2026-05-12-plan-save-protocol.md` before any code changes. Motivating incident: a 705-line plan drafted May 10 lived only in `~/.claude/plans/` (ephemeral IDE storage) and was nearly lost on context compact — Plan-Save Protocol closes this gap by making the project repo the source of truth for plans, not the IDE. Flow doc 16 updated; README install count 29→30 scripts. Reflex cycle-11/pass-001 audited the release: 5 critical/major findings fixed on the dev branch (install.sh registration, idempotency fix, +14-test F73 lifecycle round-trip, sync-check assertion, README install-count clarification). **2351 tests** (2206 framework + 145 benchmarking).

**v0.6.52 (Reflex cycle-10/pass-003 close: ARD content correction + doc-code-diff */basename fix + flow doc count sweep):** ARD Technical Overview corrected — v0.6.51 section had wrong content (PSK019 work); replaced with correct pass-002 merge content; missing v0.6.50 entry added. `reflex/lib/doc-code-diff.sh` */basename pattern fix: bare basename case patterns never matched full `$ref_clean` paths; added `*/basename` prefix to all 3 template-exclusion patterns — eliminates 4 false-positive MAJORs per reflex pass; +1 framework test (2191→2192). Test count consistency sweep: 8 files updated 2336/2191 → 2337/2192. Flow doc count corrections: `docs/work-flows/17-reflex.md` (framework count 2191→2192), `docs/work-flows/19-kit-evolution-gauntlet.md` (total count 2336→2337 in two gate boxes), `docs/work-flows/21-kit-project-evolution-loop.md` (heading `# 19` → `# 21`; `1909` → `2192` in two references). Reflex cycle-10/pass-003 artifacts committed: findings.yaml (MAJOR/MINOR closed, ADVISORY acknowledged) + 104-file history directory. Flow doc check-count correction (refresh): docs 06/11/13 updated "15 checks" → "23 checks" to match `psk-sync-check.sh` current count. **2337 tests** (2192 framework + 145 benchmarking).

**v0.6.51 (Reflex cycle-10/pass-002 merge: doc-code-diff fixes + F16 test hardening + K3/K7 cross-refs):** Fast-forward merge of `reflex/dev-cycle-10-pass-002` — 119 files committed (findings, signoff, verdict, dev-trace, 72 F-test plans, coverage, gates-result). `doc-code-diff.sh` template exclusions: added false-positive filter patterns (`foo.sh`, `psk-foo.sh`, `script-name.sh`, `test_f*_feature_name.*`, `f*-feature-name.*`). F16 test hardened: assertion changed `>=` → `==`; `00-template.md` excluded — test now asserts exactly 23 numbered docs. `23-project-update-workflow.md` K3/K7 additions: design-plan + stub creation phases, HANDOFF.md structure. AGENT_CONTEXT.md test count corrected (1982→2336). **2337 tests** (2192 framework + 145 benchmarking, +1 from doc-code-diff basename fix).

**v0.6.50 (Workflow template standard: PSK019 gate + all 23 flow docs standardised + Gate 1 scope fix):** New `docs/work-flows/00-template.md` establishes the canonical 7-section workflow template with 63-char ASCII box standard. PSK019 gate added to `psk-sync-check.sh` — enforces `## Overview`, `## Flow Diagram`, and `## Key Rules` in every flow doc. All 23 existing flow docs updated to comply; 80 misaligned box lines fixed across 13 files. Reflex Gate 1 (`preconditions.sh`) scoped to `PROJ_ROOT` subtree via `git status --porcelain -- .` — prevents false positives in workspace repos with sibling untracked directories. **2336 tests** (2191 framework + 145 benchmarking).

**v0.6.49 (resolve_kit_root bug fix: path-spaces stripping + heuristic false-positive):** Two bugs in `psk-orchestrate.sh resolve_kit_root()` fixed. (1) `tr -d '[:space:]'` stripped embedded spaces from directory names in `kit_source_path` — replaced with `sed 's/[[:space:]]*$//'` (trailing-only strip). (2) Heuristic false-positive: any project with a real copy of `portable-spec-kit.md` was incorrectly identified as the kit root — fixed by adding `[ -f "$kit_candidate/install.sh" ]` check. **2336 tests** (2191 framework + 145 benchmarking).

**v0.6.48 (Orchestrator --update mode + Phase 9 mandatory reflex + KIT_ROOT resolution + polyglot stack detection + R→F→T stubs + summary.csv GRANTED path + project-update flow doc):** `install.sh` now writes `kit_source_path` / `kit_version` / `kit_installed_at` to `.portable-spec-kit/config.md` (K1). `psk-orchestrate.sh` gains `resolve_kit_root()` (reads config → env → heuristic → remote fallback) and a full `--update` mode (U0–U10 phases: script sync → mandate audit → reflex install → design plans → feature stubs → ARD/flows → sync-check config → features → release-prep → reflex GRANTED → handoff) (K2/K3). Phase 9 reflex-audit block is now MANDATORY — no skip path; auto-installs reflex if missing (K2). Phase 7 sub-agent prompt updated to require `tests/features/` R→F→T anchor stubs alongside runnable tests (K4). `mandate-audit.sh` polyglot detection added — backend/+frontend/ projects now resolve as `polyglot-python-node` instead of `unknown` (K5). `loop.sh` GRANTED terminator now calls `score.sh` to write `summary.csv` row; cycle-09 backfilled (K6). Flow doc 23 (`23-project-update-workflow.md`) documents `--update` mode and retires the manual cascade pattern (K7). **2336 tests** (2191 framework + 145 benchmarking).

**v0.6.44 (Kit-evolution phases 1–6: config.md, template enrichment, F71/F72, test infra fixes):** `.portable-spec-kit/config.md` created and wired into `install.sh`, `psk-sync-check.sh` (PSK015-CONFIG gate), `mandate-audit.sh` (MANDATE-CONFIG-MISSING probe), and `05-mandate-compliance.sh`. All 9 agent file templates enriched with gold-standard sections (REQS approval tracking, SPECS scope-change record + `tests/features/` stub path, AGENT.md Security + DoD, TASKS.md QA Findings + Evidence, RESEARCH.md Open Research Questions, AGENT_CONTEXT.md File Structure). F71 (6 regression tests) and F72 (15 regression tests) lock in framework internal consistency and template structure completeness. PSK016 stub-path gate added. Dim 25 extended with probes 25.2–25.4. Per-feature test loop fixed (SCRIPT_DIR clobber + cwd deletion bugs). **2336 tests** (2191 framework + 145 benchmarking).

**v0.6.43 (Flow doc consistency sweep):** Step counts updated "8-step gate" → "10-step gate" in `11-spec-persistent-development.md`. Gate count "10 gates" → "11 gates" in `17-reflex.md` (PKFL added G1+G2). Dimension count "24" → "25" in `18-project-orchestration.md`. Test count "1836" → "1982" in `19-kit-evolution-gauntlet.md`. Test infrastructure flow count check 21→22. **v0.6.42 (README consistency sweep: script count + flow table row 22):** PSK013 fix: Quick Start "Installs:" updated 27→29 scripts. PSK015 fix: Flows table row 22 added for PKFL flow doc. **v0.6.41 (PKFL + convergence discipline + ARD user guide + mandate audit):** Convergence discipline fix — Rule 4 in loop.sh + run.sh. PKFL machinery: `kit-evolution.sh`, `check-kit-genericity.sh`, `smoke-test-examples.sh`; gates 10+11 in `gates.sh`; G4 `genericity_proof` gate in `file-bugs.sh`; flow doc 22; §PKFL mandatory rule; gates count 9→11. ARD user guide generation: `psk-generate-user-guide.sh` + `psk-generate-ci.sh`. 1982 tests (1837+145). See [CHANGELOG.md](CHANGELOG.md) for full history.
...
**Supply-chain integrity (OWASP A08 — accepted risk).** The one-line `curl … | bash` install executes
the installer directly from GitHub without a separate pre-execution signature check, and the installer
then downloads framework files over HTTPS. This is a standard, accepted trade-off for a zero-install,
one-line tool — the same posture as most `curl | bash` installers. The mitigations available today:

- **Inspect first** — use the *Verify before running* recipe above (`-o install.sh`, read it, then run it). Recommended for security-sensitive environments.
- **Print the installer hash** — `bash install.sh --verify` computes and prints the SHA-256 of the installer it is about to run, so you can record it for audit or compare it across machines.
- **Transport integrity** — all downloads use HTTPS (TLS) from `raw.githubusercontent.com`, so the bytes are integrity-protected in transit.

A published detached signature (GPG / a committed `install.sh.sha256` the installer auto-verifies) is a
possible future hardening; until then the exposure is documented here as an accepted risk rather than
silently ignored.
...
| **Skill-Based Architecture** | Core brain (behavioral rules, always loaded) + 29 skill files loaded JIT on demand. Core file under 1000 lines. |
...
| 20 | **[Kit ↔ Project Evolution Loop](docs/work-flows/21-kit-project-evolution-loop.md)** | Round-trip protocol for routing user-project audit findings back to the kit — preserves genericity contract, ADL/ADR routing, and bidirectional version sync between kit and consuming projects |
...
| 22 | **[Project-to-Kit Feedback Loop](docs/work-flows/22-project-kit-feedback-loop.md)** | PKFL — routes `scope:kit` QA findings from any project back to the kit, enforces 6 KGG genericity gates, then cascades the updated kit to the source project |
| 23 | **[Project Update Workflow](docs/work-flows/23-project-update-workflow.md)** | `--update` mode — brings any speckit project to kit current via U0–U10 phases (script sync → mandate audit → reflex install → design plans → stubs → ARD → config → features → release-prep → reflex GRANTED → handoff); replaces manual cascade pattern |
| 24 | **[Template-Driven Creation](docs/work-flows/24-template-driven-creation.md)** | Every non-code project file scaffolded from `.portable-spec-kit/templates/*` passing 7-criterion Quality Bar; PSK022a Template choice + PSK022b src subdir layout + PSK023 template quality gates; opt-in `src/` taxonomy (core, shared, ui, api, integrations, platform) driven by Stack table; `psk-scaffold-src.sh` populates per project |
...
| 31 | **[Sub-Agent Prompt Fidelity](docs/work-flows/31-sub-agent-prompt-fidelity.md)** | 9th reliability layer (v0.6.74, restored v0.6.77) — every sub-agent prompt cites kit rules verbatim via `kit_rule_citations:` frontmatter; rule manifest at `.portable-spec-kit/kit-rules.yml` is single source of truth; `psk-rule.sh lookup` returns verbatim text; `psk-prompt-lint.sh` + PSK042 sync-check validate citation conformance; mandatory preamble in every prompt; decision-write ban (sub-agents emit EVIDENCE, kit scripts emit DECISIONS); `psk-spawn.sh` prompt-validation gate refuses spawns with non-compliant prompts (recursive at every spawn depth); closes the sub-agent-semantic-deviation class exposed by cycle-27/28 GRANTED-without-gates incident |
| 32 | **[Instruction Fidelity](docs/work-flows/32-instruction-fidelity.md)** | 11th reliability layer (v0.6.79 narrow §Command Invocation Fidelity → v0.6.81 generalized §Instruction Fidelity) — every agent at every spawn depth executes the user's instruction exactly: no scope expansion, no scope reduction, no changed form, no reordered defaults, no preference substitution. Generic principle `instruction-fidelity-honor-exact-scope` in `.portable-spec-kit/kit-rules.yml` is single source of truth; sub-agent prompts cite verbatim via Layer 9. Mechanical sub-cases (per-script pre-flight) implement the principle for specific surfaces (`reflex/run.sh` initially); future surfaces add their own pre-flight per the retrofit guide in flow doc 32. Closes the agent-vs-user deviation class — every prior fidelity layer gated agent-vs-kit. |
| 33 | **[Session Health Indicator](docs/work-flows/33-session-health.md)** | Context-window health signal (v0.6.90+) — `psk-session-monitor.sh` maps live context occupancy to a 3-level `ctx:` badge (🟢 `<50%` · 🟡 `50–79%` · 🔴 `≥80%`), surfaced two ways: a structural Claude Code `statusLine` (always-on, agent-independent) + an agent-mirrored breadcrumb badge. Red zone fires a de-duped one-time `/clear` banner via a `UserPromptSubmit` hook. Counters the lost-in-the-middle effect; KIT-GAP-0097 migrated the hook `Stop → UserPromptSubmit` (a Stop hook returning `additionalContext` loops every turn) and derives the context window authoritatively from `compactMetadata.preTokens`. |
| 34 | **[No-Silent-Wait & Centralized Progress](docs/work-flows/34-no-silent-wait.md)** | Every long blocking op surfaces progress, so running-slow is never indistinguishable from hung (v0.6.93+) — generic heartbeat monitor `psk-progress.sh` (live file + `--status` + `--statusline`) self-wrapped via `psk-progress-selfwrap.sh`, plus the ONE centralized chunked-drive driver `psk-chunked-run.sh` (`--suite test-spec-kit`/`all-tests`/`prepare-release`, `status --table`) that runs a long op as a series of background tasks so each chunk-completion becomes a chat message. Chat messages are the universal surface (the statusLine bar is CLI-terminal-only); PSK047 enforces the monitor + driver are wired; KIT-GAP-0105→0114. |
...
- **[Work Flows](docs/work-flows/)** — 23 step-by-step flow diagrams
