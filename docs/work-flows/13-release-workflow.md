...
| **Gate** | Step 9: dual critic — `psk-sync-check.sh --full` (PSK001-PSK050, including PSK023 Template Quality Bar lint via `psk-template-quality.sh --all --strict` against `.portable-spec-kit/templates/`) + sub-agent critic (`STEP_9_VALIDATION` template) |
...
│     psk-sync-check.sh --full — now 52 checks                │
│     (PSK001-PSK050 + infra):                                │
...
│       Layer 2A: psk-sync-check.sh --full (52 checks,        │
│                 PSK001-PSK050 incl. secrets + README)       │
...
| Layer | When | What |
|-------|------|------|
| **Bash critic (sync-check)** | Steps 5, 9 + every commit + CI | 52 deterministic checks (PSK001-PSK050 incl. secrets + README drift + RFT cache + flow-doc content + critic-prompt meta-check) |
...
- **Step 9 is the terminal gate.** Nothing ships until both Layer 2A (bash sync-check, 52 checks) and Layer 2B (sub-agent critic) pass. Fabricated QUOTE lines are rejected by `grep -F` verification.
...
The helper runs:
1. **Layer 2A** — `psk-sync-check.sh --full` (52 deterministic checks, PSK001-PSK050 + infrastructure; includes PSK011 secrets scan + PSK012/013/014/015 README drift checks; RFT gate cached for 60× speedup on repeat runs)
