...
| **Gate** | Dual critic: `psk-sync-check.sh --full` (PSK001-PSK050) + sub-agent critic (`FEATURE_COMPLETE` template) |
...
- **Dual-gate validation before closing a feature.** Both the bash critic (PSK001-PSK050) and the sub-agent critic (`FEATURE_COMPLETE` template) must pass. Fabricated QUOTE lines are rejected by `grep -F` verification.
...
**Bash critic** — `psk-sync-check.sh --full` runs 52 deterministic checks (PSK001-PSK050 + infrastructure checks) including: R→F→T gate, SPECS staleness, AGENT.md Stack drift, ARD content freshness, README structural consistency (agent table row count, flow table row count, install list counts), and **secret scanning (PSK011)** that blocks commits of real-format credentials.
