...
### v0.6.39 capabilities — Playwright server-lifecycle, Next.js colocated template, template-strict scaffold, cycle persistence, cascade Kit field

Capability inventory (search keys used by psk-doc-sync.sh):
- server-lifecycle.sh (NEW reflex/lib/): starts project dev server before QA-Agent; port polled with `nc -z 127.0.0.1 $port` every 2s; PID written to `$PASS_DIR/.server.pid`; process group killed (`kill -- -$pid`) to also stop webpack/esbuild children. spawn-qa.sh calls start after Phase 0; run.sh resume-qa entry calls stop. `reflex/config.yml` gains `server_lifecycle:` block (`enabled: false` default — safe no-op for bash-only suites; enable for Next.js / React / FastAPI projects). qa-agent.md gains Live server status section: QA reads `REFLEX_SERVER_STATUS` env var (ready / timeout / disabled); hard rule — never attempt `page.goto` when status ≠ `ready`; file MINOR finding if playwright.config.* exists but status is not ready (ADR-091).
- source-structures.md: new **Next.js Full-Stack (colocated)** template — DEFAULT for single Next.js apps. ALL code under `src/` (`app/`, `components/`, `lib/`, `db/schema/migrations/`, `auth/`, `queue/`, `hooks/`, `types/`). `public/` stays at project root (framework constraint). `drizzle.config.ts out: "./src/db/migrations"`. Stack matching rule added at top: "Next.js with no separate backend → Next.js Full-Stack (colocated)." Renamed old Web App (Next.js/React) template → "Next.js Frontend-Only SPA." Added Monorepo (Turborepo/pnpm workspaces) template. Framework-managed dirs (`node_modules/`, `.next/`, `dist/`) noted as gitignored, never shown in project docs.
- psk-orchestrate.sh Phase 6 template-strict scaffold: reads PLANS.md Stack table → matches exact source-structures.md template per Stack matching rule → creates dirs exactly as shown. `drizzle.config.ts out:` auto-set to `./src/db/migrations` when Drizzle ORM in stack. Eliminates the mixed-layout pattern (app/ + components/ at root alongside src/).
- reflex/run.sh find_next_pass_dir() cycle persistence: writes `$HISTORY/.active-cycle` state file on first call; subsequent calls in the same cycle reuse the stored cycle dir. Cleared on GRANTED verdict. Fixes the bug where calling `bash reflex/run.sh single` twice in succession advanced the cycle counter unnecessarily.
- psk-version-cascade.sh Phase Q Kit field: after machinery sync, updates `**Kit:**` field in each sync-target `agent/AGENT_CONTEXT.md` so the Kit badge stays in sync with the cascaded version without a separate manual edit.
