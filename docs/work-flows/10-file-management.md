...
- **PostToolUse hook validates after every Write/Edit.** `psk-sync-check.sh --quick` fires automatically after any file write — silent on clean, warns on structural drift. The pre-commit hook runs `--quick` and blocks bad commits (KIT-GAP-0075, v0.6.83 — the fast sub-second gate, so commits stay fast); the pre-push hook runs the deep `--full` and blocks bad pushes.
